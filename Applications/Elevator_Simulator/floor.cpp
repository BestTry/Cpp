...
const int Floor::FLOOR1 = 1;                            // Static integer constants that represent the floor numbers
...
bool Floor::isOccupied() const                          // Determine if floor is occupied - If person occupies, pointer is non-zero
{                                                       // Returns boolean 'true' = 1 or 'false' = 0
    return (occupantPtr != 0);                          // If 'occupantPtr' is zero, retrurns 'false' - There is no person waiting on floor,
...
void Floor::personArrives(Person* const personPtr)      // Person appears on floor - Scheduler had set random time for person to appear on floor
