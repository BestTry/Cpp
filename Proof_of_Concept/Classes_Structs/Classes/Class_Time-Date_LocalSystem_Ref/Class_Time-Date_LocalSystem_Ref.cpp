...
public:
	...
	{										// Ensures all PC_DateTime objects start in a consistent state
		year = month = day = hour = min = sec = 0;
											...
											// Placing the functions below here, I get an compiler error ???
											
	int year;								// Attributes must be public get and display functions must have direct access							
	int month;
	int day;
	int hour;
	int min;
	int sec;
											...
											// Class related functions - Can not place functions within class without compiler errror ???
void GetCurrentDateTime(PC_DateTime &);		// Acquire date and time of the local PC system 
void DisplayDateTime(PC_DateTime &);		// Display date and time of the local PC system 
void PrintUniversal(PC_DateTime &);
void PrintStandard(PC_DateTime &);
...
int main()									// Function main() begins program execution - Entry point for the program
{
	...
	for (int dsp = 0; dsp != 9; dsp++)
	{
		...
		GetCurrentDateTime(DT_Object);		// Acquire date and time of object
		DisplayDateTime(DT_Object);			// Display date and time of object

		cout << " Universal Time: ";
		PrintUniversal(DT_Object);			// Display time in universal format
		cout << " - Standard Time: ";
		PrintStandard(DT_Object);			// Display time in standard format
...
void GetCurrentDateTime(PC_DateTime& timeObj)		// FUNCTION of class PC_DateTime
{
	...
	timeObj.year  = local_time.tm_year + 1900;		// and were made public to allow access outside of this function
	timeObj.month = local_time.tm_mon + 1;
	timeObj.day   = local_time.tm_mday;

	timeObj.hour  = local_time.tm_hour;
	timeObj.min   = local_time.tm_min;
	timeObj.sec   = local_time.tm_sec;
}													// End of function 


void DisplayDateTime(PC_DateTime& timeObj)			// FUNCTION of class PC_DateTime
{
	cout << " >>>  Current PC System Date and Time Attribute   <<<" << endl;
	cout << " Date in years  : " << timeObj.year  << " \t\t " << " Dime in hours  : " << timeObj.hour << endl;
	cout << " Date in months : " << timeObj.month << " \t\t " << " Dime in minutes: " << timeObj.min  << endl;
	cout << " Date in days   : " << timeObj.day   << " \t\t " << " Dime in seconds: " << timeObj.sec  << endl;
}													// End of function 


void PrintUniversal(PC_DateTime& timeObj)			// FUNCTION of class PC_DateTime: Print time in universal-time format
{
	cout << setfill('0') << setw(2) << timeObj.hour << ":"
		<< setw(2) << timeObj.min << ":"
		<< setw(2) << timeObj.sec;
...
void PrintStandard(PC_DateTime& timeObj)			// FUNCTION of class PC_DateTime: Print time in standard-time format
{
	cout << ((timeObj.hour == 0 || timeObj.hour == 12) ?	// Display 12 or modulus of <13 ... 23> 
		12 : timeObj.hour % 12) << ":" << setfill('0')
		<< setw(2) << timeObj.min << ":"			// Obove statement strips '12' from the output
		<< setw(2) << timeObj.sec
		<< (timeObj.hour < 12 ? " AM" : " PM");
}											        // End function printStandard
